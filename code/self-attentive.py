...
def main():
    ...
    sources_raw, labels = load_csv('./data/train.csv', target_column=0, data_column=2)
    ...
    with tf.Session() as sess:
            ...
            for epoch in range(epoch_nums):
                ...
                for step in range(step_nums):
                    ...
                    if (step % display_step) == 0:
                        ...
                        test_sources_raw, test_labels = load_csv('./data/test.csv', target_column=0, data_column=2)
        ...
        # test mode
            ...
            step = 3 #for step in range(test_step_nums):
            batch_start = step * batch_size
            batch_end = batch_start + batch_size
            batch_sources, batch_labels = (test_sources[batch_start:batch_end], test_labels[batch_start:batch_end])

            A, predicts, accuracy = sess.run([model.A, model.predict, model.accuracy], 
                                                feed_dict={model.sources: batch_sources,
                                                           model.labels: batch_labels})
            accuracy_list.append(accuracy)
            a_sum = np.sum(A, axis=1)
            a_sum_softmax = softmax(a_sum)
            print("Test Accuracy: {:.5f}".format(np.mean(accuracy_list)))
            
            # Visualize
            with open("visualize.html", "w") as f:
                label_dic = ["None", "World", "Sports", "Business", "Sci/Tech"]
                for b_idx in range(batch_start, batch_end):
                    sentence = test_sources_raw[b_idx]
                    label = test_labels[b_idx]
                    tokens = token_parse(sentence)
                    
                    predict = predicts[b_idx-batch_start]
                    aa = A[b_idx-batch_start]
                    for r_idx in range(r): # 1 or r
                        a = aa[r_idx]

                        f.write('<div>')
                        f.write('<p>')
                        f.write('<span style="margin-left: 5px;background-color:rgba(0,255,0,0.4)">{}</span>\n'.format(label_dic[label]))
                        f.write('<span style="margin-left: 5px;background-color:rgba(0,0,255,0.4)">{}</span>\n'.format(label_dic[predict]))
                        for idx, prob in enumerate(a):
                            if idx < len(tokens):
                                token = tokens[idx]
                            else:
                                token = " "
                            f.write('<span style="margin-left: 5px;background-color:rgba(255,0,0,{})">{}</span> \n'.format(prob, token))
                        f.write('</p>\n')
                        f.write('</div>\n')
